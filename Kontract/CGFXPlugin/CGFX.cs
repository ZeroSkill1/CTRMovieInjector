    ...
    public sealed class CGFX
    {
            ...
            using (var br = new BinaryReaderX(input))
            {
                ...
                for (int i = 0; i < dictHeader.entryCount; i++)
                {
                    ...
                    for (int j = 0; j < ((TxObs[i].mipmapLvls == 0) ? 1 : TxObs[i].mipmapLvls); j++)
                    {
                        ...
                        Bitmaps.Add(Common.Load(br.ReadBytes((int)(Support.CTRFormat[(byte)TxObs[i].format].BitDepth * width * height / 8)), settings));
        ...
        public void Save(string filename)
        {
            ...
            foreach (var txobj in TxObs)
            {
                ...
                {
                    if (Bitmaps[bmpCount].Width != txobj.width / (1 << j) || Bitmaps[bmpCount].Height != txobj.height / (1 << j))
            ...
            using (var bw = new BinaryWriterX(File.OpenWrite(filename)))
            {
                ...
                for (int i = 0; i < TxObs.Count; i++)
                {
                    ...
                    for (int j = 0; j < ((TxObs[i].mipmapLvls == 0) ? 1 : TxObs[i].mipmapLvls); j++)
                    {
                        ...
                        bw.Write(Common.Save(Bitmaps[bmpCount++], settings));
