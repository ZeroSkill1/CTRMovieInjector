...
namespace Kontract.Image
{
    ...
    public class Common
    {
        ...
        public static Bitmap Load(byte[] bytes, ImageSettings settings)
        {
            ...
            IEnumerable<Point> points = GetPointSequence(settings);
            ...
            unsafe
            {
                ...
                foreach (Tuple<Point, Color> pair in points.Zip(settings.Format.Load(bytes), Tuple.Create))
                {
                    int x = pair.Item1.X, y = pair.Item1.Y;
                    if (0 <= x && x < width && 0 <= y && y < height)
                    {
                        Color color = pair.Item2;
