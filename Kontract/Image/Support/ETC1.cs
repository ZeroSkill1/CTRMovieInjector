    ...
    public class ETC1
    {
        ...
        public class Decoder
        {
            ...
            public Color Get(Func<PixelData> func)
            {
                ...
                {
                    PixelData data = func();
                    RGB basec0 = data.Block.Color0.Scale(data.Block.ColorDepth);
                    RGB basec1 = data.Block.Color1.Scale(data.Block.ColorDepth);
                    ...
                    foreach (int i in (_3ds_order) ? order3ds : orderNormal)
                    {
                        ...
                        int[] mod = modifiers[(i & flipbitmask) == 0 ? data.Block.Table0 : data.Block.Table1];
                        RGB c = basec + mod[data.Block[i]];
        ...
        public class Encoder
        {
            ...
            public void Set(Color c, Action<PixelData> func)
            {
                ...
                if (queue.Count == 16)
                {
                    IEnumerable<Color> colorsWindows = Enumerable.Range(0, 16).Select(j => (_3ds_order) ? queue[order3ds[order3ds[order3ds[j]]]] : queue[orderNormal[j]]); // invert order3ds
                    ulong alpha = colorsWindows.Reverse().Aggregate(0ul, (a, b) => (a * 16) | (byte)(b.A / 16));
                    List<RGB> colors = colorsWindows.Select(c2 => new RGB(c2.R, c2.G, c2.B)).ToList();
        ...
        public class Optimizer
        {
            ...
            {
                Solution soln = new Solution { blockColor = c, intenTable = intenTable };
                RGB[] newTable = new RGB[4];
                RGB scaledColor = c.Scale(limit);
                for (int i = 0; i < 4; i++)
            ...
            static Optimizer()
            {
                for (int i = 0; i < 8; i++)
                {
                    ...
                    for (int j = 0; j < 16; j++)
                    {
                        ...
                        foreach (byte k in lookup16big[i][j]) lookup16[i][k] = true;
                    }
                    for (int j = 0; j < 32; j++)
                    {
                        lookup32big[i][j] = modifiers[i].Select(mod => (byte)Clamp(j * 8 + j / 4 + mod)).Distinct().ToArray();
                        foreach (byte k in lookup32big[i][j]) lookup32[i][k] = true;
                    }
                }
            }
            #endregion
            ...
            public static bool RepackEtc1CompressedBlock(List<RGB> colors, out Block block)
            {
                ...
                {
                    RGB[] allpixels0 = colors.Where((c, j) => (j / (flip ? 2 : 8)) % 2 == 0).ToArray();
                    RGB[] pixels0 = allpixels0.Distinct().ToArray();
                    if (pixels0.Length > 4) continue;

                    RGB[] allpixels1 = colors.Where((c, j) => (j / (flip ? 2 : 8)) % 2 == 1).ToArray();
                    RGB[] pixels1 = allpixels1.Distinct().ToArray();
                    if (pixels1.Length > 4) continue;

                    foreach (bool diff in new[] { false, true })
                    {
                        if (!diff)
                        {
                            List<int> tables0 = Enumerable.Range(0, 8).Where(i => pixels0.All(c => lookup16[i][c.R] && lookup16[i][c.G] && lookup16[i][c.B])).ToList();
                            if (!tables0.Any()) continue;
                            List<int> tables1 = Enumerable.Range(0, 8).Where(i => pixels1.All(c => lookup16[i][c.R] && lookup16[i][c.G] && lookup16[i][c.B])).ToList();
                            if (!tables1.Any()) continue;

                            Optimizer opt0 = new Optimizer(allpixels0, 16, 1);
                            Solution soln0 = null;
                            foreach (int ti in tables0)
                            {
                                int[] rs = Enumerable.Range(0, 16).Where(a => pixels0.All(c => lookup16big[ti][a].Contains(c.R))).ToArray();
                                int[] gs = Enumerable.Range(0, 16).Where(a => pixels0.All(c => lookup16big[ti][a].Contains(c.G))).ToArray();
                                int[] bs = Enumerable.Range(0, 16).Where(a => pixels0.All(c => lookup16big[ti][a].Contains(c.B))).ToArray();
                            ...
                            Optimizer opt1 = new Optimizer(allpixels1, 16, 1);
                            foreach (int ti in tables1)
                            {
                                int[] rs = Enumerable.Range(0, 16).Where(a => pixels1.All(c => lookup16big[ti][a].Contains(c.R))).ToArray();
                                int[] gs = Enumerable.Range(0, 16).Where(a => pixels1.All(c => lookup16big[ti][a].Contains(c.G))).ToArray();
                                int[] bs = Enumerable.Range(0, 16).Where(a => pixels1.All(c => lookup16big[ti][a].Contains(c.B))).ToArray();
                                Solution soln1 = opt1.FindExactMatches(from r in rs from g in gs from b in bs select new RGB(r, g, b), modifiers[ti]).FirstOrDefault();
                                if (soln1 != null)
                                {
                        ...
                        {
                            List<int> tables0 = Enumerable.Range(0, 8).Where(i => pixels0.All(c => lookup32[i][c.R] && lookup32[i][c.G] && lookup32[i][c.B])).ToList();
                            if (!tables0.Any()) continue;
                            List<int> tables1 = Enumerable.Range(0, 8).Where(i => pixels1.All(c => lookup32[i][c.R] && lookup32[i][c.G] && lookup32[i][c.B])).ToList();
                            if (!tables1.Any()) continue;

                            Optimizer opt0 = new Optimizer(allpixels0, 32, 1);
                            List<Solution> solns0 = new List<Solution>();
                            foreach (int ti in tables0)
                            {
                                int[] rs = Enumerable.Range(0, 32).Where(a => pixels0.All(c => lookup32big[ti][a].Contains(c.R))).ToArray();
                                int[] gs = Enumerable.Range(0, 32).Where(a => pixels0.All(c => lookup32big[ti][a].Contains(c.G))).ToArray();
                                int[] bs = Enumerable.Range(0, 32).Where(a => pixels0.All(c => lookup32big[ti][a].Contains(c.B))).ToArray();
                            ...
                            Optimizer opt1 = new Optimizer(allpixels1, 32, 1);
                            foreach (int ti in tables1)
                            {
                                int[] rs = Enumerable.Range(0, 32).Where(a => pixels1.All(c => lookup32big[ti][a].Contains(c.R))).ToArray();
                                int[] gs = Enumerable.Range(0, 32).Where(a => pixels1.All(c => lookup32big[ti][a].Contains(c.G))).ToArray();
                                int[] bs = Enumerable.Range(0, 32).Where(a => pixels1.All(c => lookup32big[ti][a].Contains(c.B))).ToArray();
                                foreach (Solution soln0 in solns0)
                                {
                                    IEnumerable<RGB> q = (from r in rs
                                    ...
                                    Solution soln1 = opt1.FindExactMatches(q, modifiers[ti]).FirstOrDefault();
                                    if (soln1 != null)
                                    {
                                        block = new SolutionSet(flip, diff, soln0, soln1).ToBlock();
                                        return true;
            ...
            public static Block Encode(List<RGB> colors)
            {
                ...
                SolutionSet bestsolns = new SolutionSet();
                foreach (bool flip in new[] { false, true })
                {
                    RGB[][] pixels = new[] { 0, 1 }.Select(i => colors.Where((c, j) => (j / (flip ? 2 : 8)) % 2 == i).ToArray()).ToArray();
                    foreach (bool diff in new[] { false, true }) // let's again just assume no diff
                    {
                        Solution[] solns = new Solution[2];
                        int limit = diff ? 32 : 16;
                        int i;
                        for (i = 0; i < 2; i++)
                        {
                            int errorThreshold = bestsolns.TotalError;
                            if (i == 1) errorThreshold -= solns[0].error;
                            Optimizer opt = new Optimizer(pixels[i], limit, errorThreshold);
                            if (i == 1 && diff)
                            {
                        ...
                        if (i == 2)
                        {
                            SolutionSet solnset = new SolutionSet(flip, diff, solns[0], solns[1]);
